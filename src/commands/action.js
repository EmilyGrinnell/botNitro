...
module.exports = function(message, args) {
    ...
    switch (args[0].toLowerCase())
    {
        case "add":
            args[2] = args[2] || "";
            if (!fs.readdirSync(`${path.relative("./", __dirname)}/../actions/`).getValues(item => item.split(".").slice(0, -1).join(".")).includes(args[2].toLowerCase())) return message.channel.send(`That's not a valid action. You can see a full list of actions by using \`${this.config.prefix}action help\``).catch(() => null);
        ...
        case "view":
            ...
            else return new this.lib.List(this.config.actions[args[1].toLowerCase()].getValues(item => {return {name : item[0], value : item[1].length > 1024 ? `${item[1].substring(0, 1021)}...` : item[1]}}), message.channel, message.author.id, this.client);
            //Send defined actions as a list
        case "help":
            return new this.lib.List(fs.readdirSync(`${path.relative("./", __dirname)}/../actions/`).getValues(item => item.split(".").slice(0, -1).join(".")).getValues(item => {return {name : item, value : `\`${require(`../actions/${item}.js`).desc.join("`\n")}`}}), message.channel, message.author.id, this.client);
